...
from PyQt5.QtWidgets import (
    ...
    QFormLayout, QDateEdit, QComboBox, QDialogButtonBox, QGraphicsDropShadowEffect, QSizePolicy
)
...
# ── Full-window background widget ──────────────────────────────────────────
class BackdropWidget(QWidget):
    """Paints backdrop.png scaled to fill, with a dark vignette overlay."""
...
class TextContent(QWidget):
    def __init__(self, parent=None):
        ...
        # Big headline
        headline = QLabel("Top-notch\nlogistic services")
        headline.setFont(QFont("Segoe UI", 48, QFont.Bold))
        headline.setStyleSheet(f"color: {ORANGE}; background: transparent; line-height: 1.1;")
...
class ActionButtons(QWidget):
    def __init__(self, parent=None):
        ...
        for label in ["Manage Shipments", "View History"]:
            btn = PillButton(label)
 
            shadow = QGraphicsDropShadowEffect()
            shadow.setBlurRadius(24)
            shadow.setOffset(0, 4)
            shadow.setColor(QColor(249, 115, 22, 140))
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        # Backdrop is the actual root; everything sits on top of it
        self._backdrop = BackdropWidget(self)
        self._backdrop.setGeometry(0, 0, self.width(), self.height())
 
        # Root layout floats over the backdrop
        ...
        hero_row.setContentsMargins(48, 0, 48, 60)
        hero_row.setSpacing(32)
        hero_row.addWidget(TextContent(), stretch=3)
        hero_row.addWidget(ActionButtons(), stretch=2)
...
class ShipmentManager(QWidget):
    ...
    def _build_ui(self):
        ...
        review_btn.clicked.connect(
            lambda: self.parent_window.setCentralWidget(ShipmentReview(self.parent_window)))
